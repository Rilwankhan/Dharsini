...
if (currentTheme === 'dark') {
    // disable transitions temporarily to apply theme instantly on load
    body.classList.add('no-transition');
    body.setAttribute('data-theme', 'dark');
    themeToggle.innerHTML = '<i class="fas fa-sun"></i>';
    // force reflow then remove the no-transition class
    void body.offsetWidth;
    setTimeout(() => body.classList.remove('no-transition'), 0);
}

themeToggle.addEventListener('click', () => {
    // temporarily disable transitions so the theme change is immediate
    body.classList.add('no-transition');
    ...
    // force reflow then restore transitions shortly after
    void body.offsetWidth;
